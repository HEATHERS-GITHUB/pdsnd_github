...
def get_filters():
    ...
    while city not in ['chicago', 'new york city', 'washington']:
        city = input(
        "Please select one of the following: chicago, new york city, washington").lower()
...
def display_data(df):
    ...
    display_active = input("Do you want to see the first 5 rows of the raw data?: ").lower()
    ...
    if display_active == 'yes':
        while end_loc <= df.shape[0] - 1:
            ...
            end_display = input("Would you like to see the next 5 rows?: ").lower()
            if end_display == 'no':
                break
